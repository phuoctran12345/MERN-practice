 ...
 * Xem tất cả bookings cho Receptionist (UC 7)
 */
const BookingsSection = () => {
    ...
    // QUAN TRỌNG: Thêm optional chaining và fallback để tránh lỗi khi field undefined
    const filteredBookings = bookings.filter((booking) => {
        if (!searchTerm.trim()) return true; // Nếu không có search term → hiển thị tất cả
                                ...
                                {filteredBookings.map((booking) => (
                                    <tr key={booking._id} className="border-b-2 border-black hover:bg-yellow-50">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            >
                                                <Link to={`/dashboard/receptionist/bookings/${booking._id}`}>
